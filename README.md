...
- **AI Integration:** Uses the server-side Google Gemini API with Google Search grounding and JSON output. The provider credentials are never exposed to the frontend.
...
- **Security:** Environment variables and sensitive credentials (such as `GEMINI_API_KEY` and `JWT_SECRET`) are securely injected into the container at runtime.

## Local Setup Instructions

Follow these steps to run the application locally:

- **Environment:** Configure `GEMINI_API_KEY`, `GEMINI_API_URL`, `JWT_SECRET`, and `SPRING_DATASOURCE_PASSWORD` in the DEV environment variables. The default Gemini endpoint is `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent`.
