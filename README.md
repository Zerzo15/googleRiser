...
- **AI Integration:** Uses the server-side DeepSeek Responses API with server-side web search and JSON output. The provider credentials are never exposed to the frontend.
...
- **Security:** Environment variables and sensitive credentials (such as `DEEPSEEK_API_KEY` and `JWT_SECRET`) are securely injected into the container at runtime.

## Local Setup Instructions

Follow these steps to run the application locally:

- **Environment:** Configure `DEEPSEEK_API_KEY`, `DEEPSEEK_API_URL`, `DEEPSEEK_API_MODEL`, `JWT_SECRET`, and `SPRING_DATASOURCE_PASSWORD` in the DEV environment variables. The default DeepSeek endpoint is `https://api.deepseek.com/responses` with model `deepseek-v4-flash`.
